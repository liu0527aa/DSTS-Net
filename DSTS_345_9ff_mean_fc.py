...
res_dir = '_mean_max_9ff_attention_fc_results'
class VQADataset(Dataset):
    def __init__(self, features_dir='CNN_features_KoNViD-1k_5c/', index=None, max_len=240, feat_dim=4096, scale=1):
        super(VQADataset, self).__init__()
        self.features = np.zeros((len(index), max_len, feat_dim))
        self.length = np.zeros((len(index), 1))
        self.mos = np.zeros((len(index), 1))
        # self.fps = np.zeros((len(index), 1))
        for i in range(len(index)):
            ...
            # self.fps[i] = np.round(np.load(features_dir + str(index[i]) + '_fps.npy'))
        self.scale = scale  #
...
class ANN(nn.Module):
    def __init__(self, input_size=7168, reduced_size=128, n_ANNlayers=1, dropout_p=0.5):
...
class first_Bottleneck(nn.Module):

    def __init__(self, reduced_size=128, hidden_size=32):
        super(first_Bottleneck, self).__init__()
        self.fc=nn.Linear(reduced_size, 32)
        self.frnn = nn.GRU(hidden_size, hidden_size, batch_first=True)
...
class forward_Bottleneck(nn.Module):

    def __init__(self, reduced_size=32, hidden_size=32):
        super(forward_Bottleneck, self).__init__()
        self.fc=nn.Linear(reduced_size, 32)
        self.frnn = nn.GRU(hidden_size, hidden_size, batch_first=True)
        self.relu = nn.ReLU(inplace=True)
...
class reverse_Bottleneck(nn.Module):
    ...
    def __init__(self,reduced_size=32, hidden_size=32):
        ...
        self.fc = nn.Linear(reduced_size, 32)
        self.rrnn = nn.GRU(hidden_size, hidden_size, batch_first=True)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x= self.fc(x)
        x_r =x[:,torch.arange(x.size(1) - 1, -1, -1), :]
        identity = x_r
        out , hn = self.rrnn(x_r)
        out =out +identity
        out = self.relu(out)

        return out

class DSTS(nn.Module):  # 时间记忆模型
    def __init__(self, input_size=4096, reduced_size=128, hidden_size=32, dropout_p=0.5):
    ...
    def forward(self, input, input_length):
        ...
        outputs = self.layer1(input)
        q = torch.mean(outputs, dim=1)
        q = self.q(q)  # frame quality
        # score = torch.zeros_like(input_length, device=q.device)  # score等于128维度
        return q
    ...
    def _make_layer(self):
        layers = []
        layers.append(first_Bottleneck())

        layers.append(reverse_Bottleneck())
        layers.append(forward_Bottleneck())
...
if __name__ == "__main__":
    ...
    if args.database == 'KoNViD-1k':
        features_dir = 'CNN_features_KoNViD-1k_mean_max/'  # features dir
        datainfo = 'data/KoNViD-1kinfo.mat'  # database info: video_names, scores; video format, width, height, index, ref_ids, max_len, etc.
    if args.database == 'CVD2014':
        features_dir = 'CNN_features_CVD2014_5c/'
        datainfo = 'data/CVD2014info.mat'
    if args.database == 'LIVE-Qualcomm':
        features_dir = 'CNN_features_LIVE-Qualcomm_5c/'
    ...
    index = Info['index']  # 作者自带的随机索引，共1000组
    index = index[:, args.exp_id % index.shape[1]]  # np.random.permutation(N)
    ref_ids = Info['ref_ids'][0, :]  #
    max_len = int(Info['max_len'][0])
    trainindex = index[0:int(np.floor((1 - args.test_ratio - args.val_ratio) * len(index)))]
    testindex = index[int(np.ceil((1 - args.test_ratio) * len(index))):len(index)]
    train_index, val_index, test_index = [], [], []
    for i in range(len(ref_ids)):  # 划分
        train_index.append(i) if (ref_ids[i] in trainindex) else \
            test_index.append(i) if (ref_ids[i] in testindex) else \
                val_index.append(i)

    scale = Info['scores'][0, :].max()  # 该数据库最大视频得分，用标准化归一
    train_dataset = VQADataset(features_dir, train_index, max_len=max_len, feat_dim=feat_dim, scale=scale)
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True)
    val_dataset = VQADataset(features_dir, val_index, max_len=max_len, feat_dim=feat_dim, scale=scale)
    val_loader = torch.utils.data.DataLoader(dataset=val_dataset)
    if args.test_ratio > 0:
        test_dataset = VQADataset(features_dir, test_index, max_len=max_len, feat_dim=feat_dim, scale=scale)
        test_loader = torch.utils.data.DataLoader(dataset=test_dataset)

    model = DSTS(input_size=feat_dim).to(device)  #



    if not os.path.exists('models_5c' + res_dir):
        os.makedirs('models_5c' + res_dir)
    trained_model_file = 'models_5c' + res_dir+ '/{}-{}-EXP{}'.format(args.model,
                                                                                                 args.database,
                                                                                                 args.exp_id)
    if not os.path.exists('5c' +  res_dir):
        os.makedirs('5c' + res_dir)
    save_result_file = '5c' + res_dir + '/{}-{}-EXP{}'.format(args.model,
                                                                                                     args.database,
                                                                                                     args.exp_id)
        # writer = SummaryWriter(log_dir="{}/EXP{}-{}-{}-{}-{}-{}-{}.format(args.log_dir, args.exp_id, args.database, args.model,args.lr, args.batch_size, args.epochs,datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y"))")

    criterion = nn.L1Loss()  # L1 loss
    optimizer = Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = lr_scheduler.StepLR(optimizer, step_size=args.decay_interval,
                                    gamma=args.decay_ratio)  # 等间隔调整学习率，每20次，下降为0.8
    ...
    for epoch in range(args.epochs):
        ...
        for i, (features, length, label) in enumerate(train_loader):
            ...
            optimizer.zero_grad()  #
            outputs = model(features, length.float())  # length为视频长度
